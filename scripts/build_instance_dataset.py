...
#
# 2026-08-23 第十五轮：贴边界行残条清除（strip_rule_residue）落地。
#   11/91 图块内容变化、ID 一个没丢，**没有标签要改**——被抹的全是
#   边缘毛点与真残条（絡 左侧整条虚线链、熙 右缘断裂界行），提交前已
#   逐块红色高亮目检，无一触及真笔画。
#
# 2026-08-23 第十四轮：围栏放宽档（磨损/弯折界行也能当墙）+ 行相位「盖不住
#   内容」惩罚落地。52/91 图块内容变化、1 个 ID 消失。缺陷类 11 个变化全看
#   + clean 全量对照：**没有标签要改**；4 个实例因所在页相位治愈而整格
#   平移，按 **y 坐标对齐**重定位（65:3:0→3:2「簿」、65:4:10→4:12「妙」、
#   vol02/37:1:18→1:19、vol02/20:1:2→1:4「注」，均视觉比对确认同字）。
#   教训：整页相位平移 k 格 ≠ 格序号全体 +k——margin 格会吞掉部分平移，
#   38 个平移页里只有 4 页的金标格位真的变号，必须按 y 对齐不能按相位差推。
#
...
LABELS: dict[str, list[str]] = {
    "clean": [
        ...
        # 65:3:0 → 65:3:2 第十四轮重定位：页 65 行相位长期锁在坏解
        # （相位 403、末两格悬在页外），盖不住内容惩罚修好后整页格位
        # 上移两格，「簿」从 idx0 挪到 idx2（视觉比对确认同一个字）
        "12:9:15", "64:5:19", "65:3:2", "84:8:2", "vol02/180:3:7",
        "vol02/16:2:2", "vol02/151:4:19", "vol02/47:6:17", "vol02/173:5:3",
        "vol02/169:6:5", "vol02/76:7:20", "vol02/36:5:15",
        "vol02/47:1:16", "vol02/89:2:7", "vol02/20:1:4", "66:3:3",
    ...
    "contaminated": [
        ...
        # 第三轮转入
        "65:4:12",   # 右缘整条界行竖线（第十四轮 4:10→4:12：页 65 相位治愈后按 y 对齐重定位）
    ...
    "not_text": [
        # 第十二轮补样
        "170:3:20",        # 末格位整条黑污/版框
        "vol02/37:1:19",   # 列尾空格位只剩零星碎屑（第十四轮 1:18→1:19：相位治愈重定位）
        "88:4:19",         # 底部版框横线带脏斑
