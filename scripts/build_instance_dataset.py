...
#
# 【2026-08 重标记录】切分改动后图块内容会变，标签必须跟着重看。
# 本轮 `segment` 加入列型分类、`extract` 加入 boundary_ink 之后，2771 个
# 图块内容发生变化，其中落在本表里的有 15 个：
#   · 12 个图块内容变了 → 对着当前输出重标（见下方各类）；
#   · 3 个 ID 已不存在（102:9:4 176:5:6 189:5:7）→ 移出本表。
# 重标结果：切分本身变好了——原 2 个 truncated、1 个 not_text、3 个
# contaminated 现在都已是完整干净的本字；新出现 1 个混入界行竖线
# （200:1:3）、1 个空位吃进版框线（91:3:0）。
LABELS: dict[str, list[str]] = {
    "clean": [
        "168:2:6", "138:1:5", "165:8:3", "77:8:13", "177:2:1",
        ...
        # 重标转入：切分改动后已是完整干净的本字
        "189:7:2", "203:2:4", "203:8:3", "137:9:9", "137:8:9",
    ],
    # 混入界行竖线 / 版框 / 上下邻字残余
    "contaminated": [
        "199:8:3", "31:6:10", "188:8:3", "87:1:18",
        "113:8:6", "90:2:6", "153:9:12", "103:5:5",
        "126:5:2", "65:4:10", "159:6:5",
        # 重标转入：图块左缘吃进整条界行竖线（9×134 全高连通体）
        "200:1:3",
    ],
    # 本字的墨被切掉一部分
    # 本轮重标后本批样本已无截断实例——原来的两个都已切全。这不代表全书
    # 没有截断，只说明这 67 个样本覆盖不到，需要另行抽样补充。
    "truncated": [],
    # 版框角/整条横线/空格位，根本不是字
    "not_text": ["135:5:20", "130:7:0", "179:5:20", "205:9:0",
                 # 重标转入：空格位，整块只有一条 7×132 的版框竖线
                 "91:3:0"],
}
