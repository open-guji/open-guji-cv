...
#
# 2026-08-23 第十三轮：逐带围栏（弯界行）+ 外包络裁切 + jiazhu 掩蔽落地，
#   cell_bands 波及 68% 的列，83/91 图块内容变化、ID 一个没丢。缺陷类 15 个
#   全看 + clean 抽 10：只有 105:5:2 由 contaminated 转 clean（下一字「講」
#   的头不再被裹进来，本字完整）；66:3:3 格序号又换了字（購→鏐，仍 clean）。
#
# 2026-08-23 第十二轮补样（30 个）：缺陷分母掉到 6 之后所有比值指标都在小
#   样本噪声里，且 boundary_ink 单独命中的精确率一直没量过。抽样构成：
#   仅 boundary_ink 命中 16（两册各半）、无标记对照 8、多 flag 6。
#   判读结果：boundary_ink-only 16 个里 12 个是**完整干净的高字**（得/墳/
#   訓/漢/是/佚…——vol02 字高/格高中位 0.945，字天然顶到边缘带），真缺陷
#   只有 4 个 → 单独命中精确率 ~25%。无标记对照 8 个全干净（好消息：
#   没有漏报的证据）。样本 61 → 91，缺陷 6 → 15。
#
...
LABELS: dict[str, list[str]] = {
    "clean": [
        ...
        # 第十三轮转干净
        "105:5:2",
        # 第十二轮补样：boundary_ink-only 里的干净高字 + 无标记对照
        "12:9:15", "64:5:19", "65:3:0", "84:8:2", "vol02/180:3:7",
        "vol02/16:2:2", "vol02/151:4:19", "vol02/47:6:17", "vol02/173:5:3",
        "vol02/169:6:5", "vol02/76:7:20", "vol02/36:5:15",
        "vol02/47:1:16", "vol02/89:2:7", "vol02/20:1:2", "66:3:3",
        "103:9:6", "154:8:14", "180:5:3", "8:4:8",
        "106:2:4",   # 职名列的「臣」：小字带一个邻字墨点，按惯例仍 clean
    ...
    "contaminated": [
        # 第十二轮补样
        "27:8:19",         # 「尾」底部裹进版框横线与上字残尾
        "90:8:2",          # 职名拉开列：「延」吃进下一字「講」的头
        "124:5:8",         # 顶部裹进上一字「鴻」的钩尾
        "164:7:11",        # 空格位蹭到隔壁列「雜/禮」的笔画
        "vol02/6:9:3",     # 顶部裹进上一字「郭」的残底
        "vol02/138:6:0",   # 顶部裹进上一字「周」的残底
        # 116:1:8 第十轮消失（该格位在新网格下不存在）
        "90:2:6",    # 跨列：左「文」右「人」分属两列
        # 第三轮转入
        "65:4:10",   # 右缘整条界行竖线
        # 105:5:2 第十三轮转 clean（逐带围栏后邻字头不再被裹进来）
    ],
    # 本字的墨被切掉一部分
    "truncated": [
