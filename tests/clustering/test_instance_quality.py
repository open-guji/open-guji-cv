...
def test_false_alarm_counted_and_lowers_precision():
    ...
    flags = {"v/1:1:0": ["bad_seg"], "v/2:1:0": ["rule_bar"]}
    r = evaluate_self_detection(gold, flags)
...
def test_any_flag_counts_as_detected():
    ...
    assert evaluate_self_detection(gold, {"v/1:1:0": ["whatever"]})["defect_recall"] == 1.0
    assert evaluate_self_detection(gold, {"v/1:1:0": []})["defect_recall"] == 0.0
