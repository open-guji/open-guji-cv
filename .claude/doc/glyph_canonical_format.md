...
| 维度 | 决定 | 理由 |
|---|---|---|
| 画布 | **256×256 固定** | 行业并没有统一标准（各数据集 64/96/128/224 都有）。选 256 的依据：①内容区 195px 覆盖各册原生分辨率——实测墨迹外接框 book9all 最大 150px、vol02 最大 193px、vol01 有 4% 超 195px（200 画布的 152px 内容区会让 vol01 10.1%/vol02 6.7% 的字走有损缩小路径，256 下 vol02 归零）；②2 的幂对当前模板匹配无所谓（匹配在派生 64×64 层，PNG 也不关心），但对将来可能的 CNN（stride-2 下采样链干净）/FFT 配准无害有小益；③匹配层 64×64 的 4 倍余量。存储 +64% 像素，PNG 压缩后可忽略 |
| 文件格式 | **8-bit 灰度 PNG，白底(255)黑字** | 无损；管线裁切本来就近二值（s6 产物 0/255），存灰度对它无成本；扫描/渲染来源保留灰度信息，二值化交给下游（Sauvola 是匹配层的事，参数会演进，真源不预烧） |
| 缩放 | **等比、只缩不放**：墨迹外接框超出内容区（256×(1−2×0.12)=195px）才 INTER_AREA 缩小，否则保持原生像素 | 实测结论（见 §3）：放大重采样 + 下游再二值化会让 pairs 金标的 coverage 判定翻转 10/60；只缩不放降到 1/60、逐实例自扰动 0。代价是画布内墨迹大小不完全统一——可接受，因为匹配层（normalize_patch）反正要做自己的缩放居中，相对字号元数据在 instances 表（width/height/bbox）也没丢 |
...
画布尺寸后调整一次：最初定 200×200，同日改为 256×256——从 git 历史恢复
原始图块按 256 重跑迁移（book9all 无一图块被缩过，两次转换均无损），
验证结果与 200 时逐项一致（金标同一对贴线抖动、自查询 94/94）。动机
见 §1「画布」行：内容区 195px 让 vol01/vol02 的有损缩小比例从
10.1%/6.7% 降到 4%/0%，外加 2 的幂的余粮。

## 4. 对外部来源的约定（P1 起适用）

- **字体渲染**：直接渲染到 ≤195px 字面并留足白边（画布 ≥256），
  `to_canonical(clean=False)`。不要先渲小图再依赖放大。
- **字典扫描**（康熙等）：先完成切分与去噪（等价 phase4 的职责），
  canonical 化时 `clean=True` 兜底。
- 高分辨率来源（>195px 字面）会被 INTER_AREA 缩小——这是唯一有损路径，
  可接受（信息只多不少）。
