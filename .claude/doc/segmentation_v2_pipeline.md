...
**测试集**：`open-guji-dataset/border-detection`，**11 页**（普通页
vol01/137、138、24、65 + 曾以为是抬头、核校后确认不是的 vol01/9、14、
142、141 + 真抬头页 vol01/32、33、49）。图源用 `rebuild_src` 原始
...
**算法准确度**（`detect_borders()` vs 11 页金标，逐条线在两个自然
端点算坐标误差，`竖直线`/`水平线` 分开统计，脚本未入库、跑法见下）：

| | 基线（原 `find_horizontal_border`，整带单峰，5页） | 窄窗口第二候选（5页） | 窄窗口第二候选（11页） |
|---|---|---|---|
| top 均值/最大 | 8.5px / 39.8px | 1.1px / 6.9px | **0.5px / 6.9px** |
| bottom 均值/最大 | 14.6px / 43.2px | 8.9px / 27.5px | **7.0px / 38.9px** |
| 竖直线均值/最大 | 2.6px / 19.2px | 2.6px / 19.2px | **2.0px / 19.2px** |

竖直线（`find_vertical_lines`）本身已经很准（前述 vol01/141 去重bug
修完后11页均值2.0px），水平线基线有两类失败：
