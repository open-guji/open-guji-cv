...
### 4.2b 页面结构：四板块（2026-09-18 落地）

§4.2 的「七个视图」是按**功能**切的（产物/评测/审查各一个 tab）。实际落地
按 **Step** 切（Step0~9 各一页），于是"每个 Step 页面内部长什么样"成了新问题
——盘点时十个页面**没有一个四块齐全**，页范围有四套来源，总览有多份重复实现。

现在固定成四板块，顺序不变（`components/common/StepLayout.tsx`）：

| 板块 | 职责 | 组件 |
|---|---|---|
| ① 页范围 | **唯一源**，对下面所有板块生效；取值缓存在 localStorage | `PageRangeSelector` |
| ② 总览 | ①范围内的闸摘要 / 该步专属指标 | `ProgressGatePanel`（产物新鲜度另有 `StepStatusSummary`，两者数据源不同） |
| ③ 裁决台 | **只做可裁**：出可疑卡 → 收裁决 → 提交；多个时用 tab | 各步自己的面板 |
| ④ 产物台 | **只读**，支持多种粒度：按页看叠图（`ProductViewer`）/ 按坐标点名看（`CellLookupPanel`） | — |

两条要点：

- **裁决台与产物台的分界是「可裁 vs 只读」，不是「出卡 vs 浏览」。**
  自由浏览（看没问题的、按坐标 debug）归板块④。让两处都能提交的话，
  批次归属、touched 集合、已裁去重这三件事就要各维护一套。
- **页范围唯一源**是这轮最实的收益：此前 Step3 的拖切线台与 Step7 的切分
  裁决台各用各的页范围，同一批用例看起来「两个台不重合」，而机制上
  `blocking ⊆ all` 恒成立。统一后三处数字对上（bxgb 全书 all=916 / blocking=4）。
  面板接可选 `pages` prop：页面传了以它为准，不传退回自管——非 Step 页面照用。

不套这个骨架的例外，都有据可依：Step6（LLM 调用日志）、Step8（收割台账）
与页无关；Step9 没有对应的后端 Step id（它是现场渲染，不是「选一页看产物」）。

### 4.2c 裁决的分类真源：`question`

原设计里「这条裁决属于哪一类」靠 `kind`（受控枚举）承担。实际演化出三重拼凑：
`kind` + 卡片 id 前缀（`head:` / `outer:` / `cols:`）+ `payload.v`。三者都不是
为分类设计的，于是同一个 `kind` 承载了不同语义——实测后果是 **70 条「这页有没有
抬头」(yes/no) 混进了问「界行在不在缝上」(ok/miss/extra) 的金标分片**，
档位都对不上，谁拿它评测谁中招。

现在 **一个 `question` = 一个问题**（`feedback/questions.py`）：

```
question = "<step>.<对象>.<问什么>"     # border_detect.page.has_head_raise
```

每个 question 自带答案枚举、金标分片、expected 取哪些键。`kind` 退化成传输层
的粗分类，不再承担语义。前端随裁决发 `payload.question`，路由据此分流。

**登记表只管声明式的三要素**；那些靠踩坑换来的转换规则（`n_raised` 只在
`raised=="yes"` 时有意义、下版框要用 `y_left_abs` 而非相对坐标、`char_*` 在
2026-09-13 之前存的其实是 `shape`）留在 `consumers._expected_of` 里，连同解释
「为什么」的注释一起——硬塞进数据结构会把理由丢掉，而那才是这些规则的价值。
