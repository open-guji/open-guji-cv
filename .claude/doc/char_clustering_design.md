...
```json
{"cluster_id": "c00042",
 "candidates": [{"char": "通", "p": 0.83, "sources": ["glyph_knn", "ocr"]},
                 {"char": "遇", "p": 0.09, "sources": ["ocr"]},
                 {"char": "過", "p": 0.04, "sources": ["ocr"]}],
 "source_detail": {"glyph_knn": {"hit": "g_000381", "f1": 0.87}, "...": "..."}}
```

- **异体字归一**：候选合并前过异体字映射表（OpenCC + 自维护 `config/dicts/` 变体表），
  "为/爲/為"计入同一候选并保留原形体，避免票数被变体稀释。
...
3. **beam search 解码**：`score = λ·log P_ocr(c) + (1-λ)·log P_lm(c | context)`，
   beam 宽度默认 8，λ 默认 0.55（标定见 M7）；跨列衔接：同页相邻列首尾相连
   （古文不分词断句，n-gram 直接跨列滚动是合理近似；夹注/表格页面例外，按 profile 关闭跨列）；
4. **后验与边际**：对每槽位输出归一化后验 `p(c)` 及 `margin = p(1st) - p(2nd)`。
...
每个 `(char, edition_tag)` 存代表字形（簇 medoid），同字不同版可多条 —— 字形库
同时是"某书版的字样档案"，本身就有版本学价值（未来可做刻工/书版比对）。
