...
| 规律/问题 | 对策 |
|----------|------|
| "跨列行对齐"在写刻本**不成立**——手写上版每列行位浮动可达半字 | 模型改为**页级共享字高 + 每列独立相位** |
| 列内字距局部不均，刚性等距网格误差沿列累积（半格错位） | **弹性网格 DP**：恰好 N 格硬约束 + 每格 ±15% 伸缩 + 偏离标准字高二次惩罚，全局最优选线 |
...
结果：book9 十页 2100 字，之×10+、書×10+、進×7、中×5 等 96 个真字多成员簇
零污染；bad_seg 从 book5 的 35% 降到 0.05%。中间 6/8 列网格线全部精确落在字间。
