...
队列行的 `match` 是 seed 当时对**当时那个库**算的快照——这一轮进库的
字形，后面页的待审行没见过。库每轮都在长（本轮 7183 → 7289 条、
1202 个不同字），不重匹配就等于「刚入库的都没用上」。

固定动作：每轮入库后对**下一批要审的页** `seed --force-pages`（一次十页）。
`readjudicate_pending` 顶不了这个用——它只拿存量快照按新规则复裁，
不重新查库。两件事都要做，顺序是：先重匹配（换新证据），再复裁（换新规则）。
