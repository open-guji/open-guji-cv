...
2. ~~笔宽归一~~ **已于 2026-08-24 撤除**（§六.3）。当时预估的 0.71~0.74
   ...
   **下一个同类杠杆是提分辨率**（NORM_SIZE 64 → 96/128）：早期扫参时它与
   撤笔宽归一同量级，但代价一样是重建冻结集（char-clustering / pairs 的
   crops、char-normalization 的 golden），而且 64×64 还写死在
   `eval_clustering.py` 里。没量过它在撤除笔宽归一之后还剩多少增量。
...
1. **库匹配覆盖率的坑**（§六）：撤除笔宽归一后已填回大半
   ...
   下一个同类杠杆是提分辨率（§四.2）。在那之前，若某次更在乎吞吐而不在乎排序质量，
...
6. **聚类侧用不上 never-match 护栏**：聚类是无监督的、拿不到字标签，
