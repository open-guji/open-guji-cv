...
（`config/crop_exclusions.jsonl`，753 条 = 人裁 39 + 判据旗标 714）。
四个评测脚本默认跳过名单里的实例，`--include-excluded` 能把旧口径量回来。
...
| 护栏 | 全量口径（旧） | **排除口径（新默认）** |
|---|---|---|
| triplets control | 1.0000（60 组）| 1.0000（53 组）|
| triplets hard | 0.7632（38 组）| **0.7879（33 组）** |
| pairs 主指标 | 0.1444 @闸 0.9945 | 0.1241 @闸 0.9955 |
| pairs 固定闸 0.996 的 recall | 0.1050 | **0.1106** |
| pairs `random` 层（无偏）recall | 0.0324 | **0.0453** |
| 聚类 purity | .99967 / .99901 / 1.0 | .99965 / .99886 / 1.0 |
| 聚类碎片率 | 2.77 / 2.96 / 2.58 | **2.65 / 2.77 / 2.58** |
| eval_db_match 覆盖 | 8.7 / 16.3 / 18.1% | **8.8 / 16.7 / 18.6%** |
| eval_db_match 字形精度 | 1.0000 ×3 | 1.0000 ×3 |
| glyph.db | 2530 实例 / 706 字头 | **2338 / 673** |

主指标那一行看着退了，是**构成变了不是算法退了**：同口径下固定闸的 recall
0.1050 → 0.1106、无偏层 0.0324 → 0.0453（+39%），都在涨。

> ⚠️ **副作用：`tier=degraded` 这一层基本没了。** 排除名单用的就是当初
