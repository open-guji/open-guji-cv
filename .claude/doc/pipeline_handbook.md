...
**修法方向（并入预处理专项，与 §8 的 s3 一起做）**：
1. enhance_lines 画线前要求原图证据（该 x 邻域有足量断续墨）；
2. 或评估直接关掉 s4——phase2/segment 现在自带分带界行检测，
   可能已不需要描线辅助（关掉需全书重跑验证）；
3. 兜底：chars 层对「完美长线」指纹做列内检测 → 标 rule_bar 级确定层。
