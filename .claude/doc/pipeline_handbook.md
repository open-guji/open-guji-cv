...
| M | **字形匹配** | `GlyphMatcher`（normalize→HOG kNN→verify_pair_elastic）| cov + same/unsure/diff | `glyph-match/triplets`（98 三元组）| hard 38 条 rank_acc **0.079 → 0.684**（elastic 判据，2026-08-24）、control 60 条 1.0 护栏未动；剩下 12 条失败以 已/巳、日/曰 这类整字近似家族为主，下一个大杠杆是**去掉笔宽归一**（要重建两个冻结集）；栈的完整交接 → **[glyph_match_stack.md](glyph_match_stack.md)** |
