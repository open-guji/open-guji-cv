...
3. **切分线索不是字号，是版式**：康熙字典释文是**双行夹注**，字头是单行满宽。
   ...
   `doc/_archive/jiazhu_detection.md` 已有的原语，不用新造轮子。粗糙实验（同文书局
...
---
## 9. 候选速查释义层（2026-08-24 实现）

审阅界面每个候选按钮现在带：拼音/注音/反切 + 一行释义（悬停全文）+
候选间关系徽记（`異體:爲` / `通假:乃`）。数据两张表：

**释义表** `config/gloss/gloss.json`（8MB，`scripts/build_gloss.py` 构建），
四层合并（先到先得），每条带来源标签，授权署名见 `config/gloss/README.md`：

| 层 | 源 | 授权 | 贡献字数 |
|---|---|---|---|
| 1 | 萌典/重編國語辭典（中文白话+注音拼音） | CC BY-ND 3.0 TW（首义项**原文照录**规避 ND） | 11,258 |
| 2 | 《開放康熙》点校文本（文言带反切书证） | CC BY-SA 3.0 | 36,475 |
| 3 | zh 维基词典（kaikki 抽取） | CC BY-SA 4.0 | 15,165 |
| 4 | Unihan kDefinition（英文兜底） | Unicode License | 3,595 |

覆盖实测：**本书语料 4,637 字 → 99.3% 有释义（98.7% 中文）**；
全字表 102,998 → 60.4%（剩下的是扩 C 以后的 IRG 字，任何词典都没有释文，
在候选里出现概率极低）。读音：拼音 44,354、注音（moe）、反切（kFanqie 2 万+
康熙释文自带）。排除的源：汉典（NC+ND）、chinese-xinhua（MIT 声明无效，
新华字典文本版权在商务印书馆）、CBETA（NC）。

**通假关系**并入 variants.json：hydzd-borrowed 2,915 对独立标签，
永不进高置信异体集合（通假是不同的词，不是同词异形）；无向存储，
方向性损失（借字→本字）记录在案。另探明 BNU tongjiazi-resources
（8,009 对带古音释义，README 自称 MIT 但释义文字引自汉语大词典）——
关系对可用、释义文本不入再分发文件，留作后续增补。

模块：`open_guji_cv/gloss.py`（`gloss_of` / `annotate`），接入点
`artifact_export._with_gloss`，表缺失时静默降级。
