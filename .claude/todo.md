...
## 🎯 `find_vertical_lines` 在灰度扫描上整本挑错线（2026-09-16 重查，⛔ 原诊断是错的）

**现象订正**：不是「11 页挑错」，是北行日錄刻本（`beixing-guben-workspace`）
**全书 47.4%（512/1080）的竖线落在字身上，48/54 页有 >2 条错线**。
连 p20 这种看着「干净」的页也是 21 条里错 18 条——只有左右版框是真的。
**错线之间间距照样 ~119px**（字格周期 = 列距周期），所以闸2 用列宽一致性
完全查不出来，「过闸 18/20」是假象。

**2026-09-15 那版诊断里有两条是错的，别再照着走**：

| 原文 | 实测 |
|---|---|
| 「框内量得到 19 条脊线、强度中位 1.00」 | **不成立**。`<128` 二值化后框内逐列墨占比，界行处只有 0.29~0.42；`1.00` 那几条是左右版框和折痕。p41 框内 NMS 后只剩 3 条。 |
| 「候选池 24~27 条里截断挑错」 | 候选池是有 24~27 条，但**真界行压根没进池**（p41 池里 22 条，真界行一条都不在）。不是挑错，是没得挑。 |

所以原方向「在截断那步并入列距规整度」**救不了**——池子里就没有正确答案。

**真根因：`score = proj / 半高宽` 在灰度扫描上反选**

```
        投影      半高宽    score
真界行   526~660   58~61     9~11     ← 投影更高，却被淘汰
字身竖向  269~458    4~11    28~113    ← 被选中
```

`half_height_score_at` 的半高宽按**绝对值 `v*0.5`** 量。这本书界行两侧紧挨着字，
投影在界行附近**从不回落到峰值一半**，半高宽从 ~3px 一路走到 58px，分数被除塌；
字身笔画的竖向对齐两侧是列间空白，投影干脆落下去，半高宽 4~11px，分数虚高。

**为什么四庫總目没事**：那是 **1-bit 双色**扫描（`np.unique(g)` 只有 `{0,255}`），
界行纯黑、列间纯白，投影是干净尖峰、半高宽 3~5px。本书是 **254 级灰度**，
界行是**浅灰细线**（沿线 min gray 中位 ~130，局部淡到 186），字是实黑（p10 52~81）——
`<128` 这一刀把界行切碎、把字完整保留，投影形态整个反过来。
`border-detection` 14 页金标（全是四庫總目）**中位误差 0.1px、139/140 在 20px 内**，
算法本身没坏，是**成像特性**触发的。

**改 `ink_threshold` 救不了**（已实测）：100/128/150/170/190/200 每一档，
真界行投影都**高于**字身列。问题不在阈值，在 `proj/半高宽` 这个打分形态。

**可用判据（已实测可分，未落地）**：界行是**细而淡的脊、两侧是列间空白**；
字身列是**一整个字宽的黑、两侧还是字**。取「±35px 邻域灰度 − 自身灰度」的脊响应：

```
ridge = 邻域均值灰度 − 自身灰度       （越大越像界行）
真界行  +41 ~ +51          字身列  −12 ~ −17
```

p20 全页 21 条逐条验过完全可分（真界行最小 +14.8、字身最大 −4.1；偏移取 ±35 起
两类不重叠）。仅换判据、picker 还很粗糙时，全书界行命中率 **60.7% → 87.7%**。

⚠️ **判据可信，但选线策略还没做对**：试过「按标称列距 snap」，会被**版心那列
天生更窄**带走（正是下面 dead end ① 说的）；改成左右半叶各自定 pitch，又卡在
版心两条线定位不稳、pitch 漂到 105/109。**这一段要专门一轮。**

**已排除的死路（沿用 2026-09-15，仍然成立）**：

1. **「等距网格」假设不成立**——版心比正文列窄，干净的 p20 拿理想网格量也偏离 22.9px。
2. **纯墨脊阈值挑线不行**——按 `frac` 局部极大 + 阈值 0.55 直接选，全书 20~43 条，
   54 页里只有 1 页恰好 21 条。要选对必须带上原有的位置+角度联合精搜。

⚠️ **这是 `keben_body_v2` 全链共用代码**，四庫總目十册也走它。改之前必须拿
`open-guji-dataset` 的 `border-detection`（14 页金标）与 `column-split`（60 页人裁，
现正确率 93.3%）全量回归，确认不退步。

**下游影响**：北行日錄那边 §三.2「过闸列数中位 18/20」「period 71.0」等结论都是在
错误线位上算的，修完要重测；字流对齐要等这条修完再动（否则字位本身是错的）。
正本见 `overview/项目进展/图片初步数字化/进度/北行日录古本/01-step2界行识别.md`，
证据图 `p20_overlay.png`（绿=真界行、红=现检测结果，红线穿字一目了然）。

（同一轮已修掉的另一条：上版框搜索带 `band_frac` 写死 0.15，天头宽的书根本够不到
版框——见 commit `4887cd8746`，现已可由 `BookSpec.top_band_frac` 配。）
