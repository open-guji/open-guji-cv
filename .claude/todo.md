...
## 🎯 `find_vertical_lines` 在候选池里挑错 21 条（2026-09-15 查清根因，未修）

**现象**：北行日錄刻本（`beixing-guben-workspace`，筒子页 20 列）全书 54 页里
**11 页**闸2 不满 18 列，最差 p41 只 3/20、p46 11/20、p40/p7 各 13/20。

**根因已查清，不是界行印得虚**：全书每页框内都量得到 **19 条脊线、强度中位 1.00**
（`frac` = 框内列墨占比，界行处贯穿到顶）。问题在 `peak_line_search.find_vertical_lines`
最后那一步——粗筛出 24~27 个候选，按**半高宽匹配分**排序截前 `expected_count`(21) 条时
**挑错了**：有的线落在字身笔画上、真界行反被挤掉。表现为列距序列出现
`60 / 69 / 212` 这种（212 = 漏掉一条真线把两列并成一列；60~78 = 捡了个笔画假峰）。
正常页列距稳定在 114~126。

判据：`p41 gaps: 253 124 126 120 178 142 105 69 212 71 171 60 102 78 120 116 120 122 116 114`
对比 `p20 gaps: 272 126 121 125 115 122 116 117 120 118 115 126 114 122 117 121 120 119 115 116`
（首个 ~260 是版框外页边，正常）。

**已排除的两条死路，别再走**：

1. **「等距网格」假设不成立**——版心比正文列窄，全书列距天生不齐；
   拿理想等距网格量，连干净的 p20 都偏离中位 22.9px，分不开好页坏页。
2. **纯墨脊阈值挑线不行**——按 `frac` 局部极大 + 阈值 0.55 直接选，全书找出
   20~43 条（字身笔画连成的竖向对齐也过阈），54 页里只有 1 页恰好 21 条。
   要选对必须带上原有的**位置+角度联合精搜**，不能只看投影。

**方向**（未验证）：在候选池截断那一步把「列距规整度」并进打分——不是要求等距，
而是**优先选出让整页列距分布最紧凑的那 21 条**（版心那一列天然窄，允许一个离群）。
