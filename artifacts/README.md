...
| Step3 截断候选复核 | https://claude.ai/code/artifact/c2597865-a793-4069-b835-3e6721c4ad1d | **HTML 快照不入库**（base64 内嵌 30 格裁切图，约 306KB，裁决自存） | 用户提议的截断判据：字若被切分线截断，切口 1~2px 内必有余墨，不用逐格人工核校精确边界，先自动初筛再人裁"这一边有没有余墨"。跟老管线 `extractor._boundary_ink_frac`（10%边缘带墨占比）是同一条思路，这次改成只看**紧贴切分线的 1~2px**（不是 10% 带）。4 列 84 格全扫：`max(top_frac,bot_frac) > 3%` 筛出 30 格候选（45 格边缘干净、9 格在阈值以下但非零）。抽样核对 5 格里 4 格是真实截断（邻字的撇捺/横画明显切过切分线），1 格是正常字缝的边界性余墨——判据有效，但阈值只是拍的，需要人裁决定最终门槛。审查页给每格上下各留 45px 上下文，画出被质疑的那条边（红=质疑/浅黄=仅上下文），四选一：完整/上余墨/下余墨/两头都有。生成脚本（未入库）`scan_edge_ink.py`（先出分布）+ `gen_truncation_candidates.py` + `build_truncation_review.py` |
