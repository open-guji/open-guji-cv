...
| Step1边框探测金标标注 第二批 | https://claude.ai/code/artifact/2b1f11ee-dfe0-4919-a81b-2b54fbf81f05 | **HTML 快照不入库**（base64 内嵌 6 页整页图，约 4.1MB，交互页会自存） | **待人工核对/保存**。扩大 Step1 金标样本量（第一批只有5页，抬头样本9例里5例还是vol01/32的推测值）。新增3个抬头候选(vol01/9、14、142——都是"上諭"引文被抬头顶格排的普通正文页，不是vol01/32那种被裁切/vol01/47-53那种带装饰花框的特殊页；用 `build_top_contact_sheets.py` 把 vol01 108个body页最上方一条带批量裁图人眼扫过，逐页目视确认边框上方有清晰字迹凸出、且页边留白够、不是花框才收进候选) + 3个新普通页(vol01/24、65、141，目视确认顶部无凸出文字)。种子/生成流程与第一批相同，脚本（未入库）`gen_border_gold_seed_r2.py` + `build_border_gold_tool_r2.py` |
