...
| **切分裁决台**（格线 A/B 盲裁）| https://claude.ai/code/artifact/89c2a1a1-009f-4178-8bdd-323cddb34684 | 64 | 同一块图两种切法并排（蓝线/黄线），判哪条把整理本那两个字切对了。4 档：蓝线对 / 黄线对 / 都不对 / 拿不准。再生 `scripts/build_cutline_flip_review.py`；身份对账表 `cutline_flip_cards.jsonl` |
...
**切分裁决台**（2026-09-12）要验的是一个更靠上游的怀疑：库匹配 diff 档带出
候选字之后，vol02 1-50 页 145 条待裁里 65 条能唯一命中整理本期望字，其中
**64 条指向「现役切错了」**，64:1 不像噪声，像 `chosen` 规则本身有问题
（`row_boundaries.py`：只要折线绕开墨且有偏移就一律选折线，**不比较 straight
自己穿多少墨**）。但判据用的是识别结果，拿它裁切分是循环依赖，所以必须人裁。
⚠️ 按出题纪律，卡上**不标哪条是现役**、左右顺序随机（实测 A 位 chosen 34 /
flip 30），真实身份只在 `cutline_flip_cards.jsonl` 里，收回后对账算准确率。
