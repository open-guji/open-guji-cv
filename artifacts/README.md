...
| 粘连点金标字对复核 | https://claude.ai/code/artifact/558abc74-ada7-41cc-a886-0229fd3b4380 | **HTML 快照不入库**（`scripts/touch_resolve/out/label_review.html`，59 张卡内嵌 16 级灰缩略图约 374KB，裁决自存） | `touching-cuts` 金标的 `char_above/char_below` 来自整理本对齐，实验一（`scripts/touch_resolve/exp1_*`）发现 41 条识别漏网里约半数是**金标字对与图像不符**（整格错位、整理本与刻本用字不同）。本页把可疑的 49 侧半字 + 10 张对照卡出题：只显示字形与 ≤3 个候选字（金标字、识别 top-1/top-2，**不标来源、顺序打散**）+「都不是」「看不清」。收回：`Artifact action:"read"` → `scripts/touch_resolve/apply_label_verdicts.py read_back.html [--apply]`（先落 workspace `feedback/touch_label_verdicts.jsonl`，再改数据集金标并记 history）。生成脚本 `scripts/touch_resolve/build_label_review.py`（2026-09-13） |
