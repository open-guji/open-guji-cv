...
| vol01 新算法逐字框选（十页） | https://claude.ai/code/artifact/6f054ade-34f7-4169-8b81-e5eaf82e0ffd | **HTML 快照不入库**（base64 内嵌 10 页整页图，约 11.5MB）| `row_boundaries.py` 已合并到 main（跟 `grid_segment.py` 并存，未接入生产管线）之后的整体效果抽查：vol01 133~142 共10个未进过任何审查工作流的正文页，版框/列线仍用生产 `phase3_char_grid` 产物，只把"一列切21格"这步换成新DP，切完后复用生产 `CharExtractor.extract_page` 缩到最小矩形（未重新发明这一步）。抬头列未做特殊处理（统一走默认21格），如混入会跟 vol01/33 一样跑偏，图上应该能直接看出来。生成脚本（未入库）`run_new_algo_pages.py` + `build_new_algo_gallery.py` |
