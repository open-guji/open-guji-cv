# -*- coding: utf-8 -*-
"""校对本里**根本没有**的段：校勘按语、卷端题、卷末题。

这些段不参与对勘——不是因为它们特殊，是因为「拿有按语的页去跟删了按语的校对本
...
不摘出去的后果：p56 整页 170 字里 160 字在校对本里不存在，8-gram 锚定拿前 5 字
「接晚過黃壁」命中校对本第 18251 字，整页按那个偏移对齐，报出 60+ 条「改/增删」
——**图与转写一个字都没错**。撰人题那 4 格则被配成 宋→州、樓→教、鑰→授、
𢰅→隨 四条假「改」外加一条 16 字 missing，一处体例差异报成五条。

## 判据：查校对本里有没有，不认「案」「按」字头

起初想按起头字认（「案」起头的是按语），但全书四段夾注的起头字是
張 / 或 / 並 / 案，只有一段是按语——**样本太少，认字头就是过拟合**。
改成直接拿这段文字去校对本里查：在就比，不在就不比。这是**可核实的事实**，
...
#: 夾注段短于这个字数就不查——「並六十陌」这类三五字的注落进校对本正文纯属
...
def mostly_absent(probe: str, corpus_norm: str, k: int = 4, hit_max: float = 0.25) -> bool:
    ...
    按「出现过」算命中率接近 1，会被当成正文放过去；但那些引文散落在校对本的不同
    位置，**没有一个连续区间能同时装下它们**，正文则一定有。

    整串精确匹配也不行：**一个字对不上就全串落空**。p24 第 1 列是正文，
    只因校对本作「廪」而刻本刻「廩」（异体表没收这对）就被误摘。

    短串（≤8 字）k-gram 太少，统计不出「大面积命中」——卷末题「北行日錄下完」
    只有 3 个 4-gram，而「北行日錄」在校对本里是书名、必然命中。短串回到整串匹配，
    它们本来就短，一个字对不上的风险低。
    """
    if len(probe) <= 8:
...
def absent_runs(cells: list[dict], corpus_norm: str, normalize) -> list[dict]:
    ...
    ——校对本作「北行日録」而刻本刻「北行日錄」，不归一会把卷端题误判成「校对本里没有」。
    """
    out: list[dict] = []
    ...
    # （字位 18,237→12,138、「刻本多」暴涨到 930）。正文列在校对本里是连续文本，
    # 但校对本有分页、有异体、有个别识别错，「整列连续命中」正文本来就满足不了。
