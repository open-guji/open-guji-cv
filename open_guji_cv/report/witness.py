...
- 长行里 19 字行占 76.4%；
- **列首落在语料行首**：vol01 93.7%（312/333）、vol02 76.2%（244/320）；
- 列长与对应行长**相等**：全量 2866 个对齐列里 2728 相等（95.2%）；
- 不等的 138 例主型是「我们 19 / 光盘 20」——**我们少一个字**，正是已知的丢格缺陷。

所以这份语料不只是字符流证人，还是**列结构证人**：不做字符比对也能查出
「这一列少了几个字」，且**独立于 difflib 怎么对齐**。`line_starts` 就是为它准备的。

⚠️ vol02 命中率明显低于 vol01，原因未查（可能抬头/敬语行多，版式留白不是 2 格）。
`col_verdict()` 对未命中的列只报 `col.drift` 供人看，**不当成错误计数**。
...
def col_verdict(col_start: int, col_len: int, w: Witness) -> tuple[str, int]:
    ...
    - `col.ok`：列首命中行首且长度相等（全量实测 95.2%）；
    - `col.short`：命中行首但我们**少**字 → Step3 丢格（138 例里的主型）；
    - `col.long`：命中行首但我们多字 → Step3 多切，或证人漏字；
    - `col.drift`：列首没落在行首——上游累积错位，或本列是抬头/标题
      （证人对这类行不一定断行）。**这条不当错误计数**，见模块头 vol02 警告。
    """
