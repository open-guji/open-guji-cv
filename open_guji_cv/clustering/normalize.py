...
SAUVOLA_WINDOW = 31
#: 2026-09-16 由 0.2 降到 0.10（用户裁定）。北行日錄刻本这类**淡墨细笔**的扫描上，
#: 0.2 太保守：细笔画大量落在灰度 140–180 这条中灰带，而 0.2 只召回其中 10.5%，
#: 笔画因此残缺到难以辨认（病例 `bxgb:4:5:4` 嚴，该字块 9.3% 的像素在这条带上）。
#:
#: 360 个真实字块（6 页）实测：
#:
#: | k    | 中灰 140–180 召回 | 纸面误判 | 连通域中位 |
#: |------|------------------|---------|-----------|
#: | 0.20 | 10.5%            | 0.000%  | 4         |
#: | 0.15 | 16.8%            | 0.000%  | 4         |
#: | 0.10 | **24.4%**        | 0.000%  | 5         |
#: | 0.05 | 32.7%            | 0.000%  | 5         |
#: | 0.00 | 41.5%            | 2.089%  | 10（塌）  |
#:
#: ⚠️ **不要改成固定阈值**：`<170` 召回 60.3% 看着最好，但全书 56 页里 p1（封面）
#: 纸峰只有 166（其余 55 页 193–209），固定阈在那一页把 68.7% 的纸判成墨、整页糊死。
#: Sauvola 是局部阈值，同一组 k 在 p1 上纸面误判仍是 0.000%。
#:
#: ⚠️ **降 k 与 `stroke_normalize` 是相反的失效方向**，别混为一谈：后者膨胀笔宽、
#: 会把 已/巳 的开口糊死（所以退役）；降 k 是把**断掉的笔画接回来**。实测 日/四/自/
#: 申/甲/回 的洞数随 k 下降**只升不降**（四 1→2、自 0→1）。到 0.05 才开始出问题
#: （吅 类开口填实、下 的左下撇尖被纸纹粘掉），所以止步 0.10。
#:
#: 这个常数是 `normalize_patch` / `to_canonical` / `seeding` / `binarize_page` 四处
#: 共用的——**播种与匹配必须同一把尺子**（见 font_template 那次教训）。改它会连字体
#: 模板一起改，但实测 22 个字 k0.2 与 k0.1 在字体渲染图上平均只差 0.18% 像素：
#: 字体本就是干净黑白，k 对它几乎不起作用。改动是单向的，只修扫描侧。
SAUVOLA_K = 0.10
