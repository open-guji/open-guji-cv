...
"""2026-09-07 重标（外部真刻本源上线，`external_glyph_sources_experiment.md` §5.8）。

旧值 0.03 是按**字体模板 + run-5 checkpoint** 的分数分布标的，那个分布里同组两形的
embedding 余弦差平均只有 **0.0138**——连阈值一半都不到，实测这条通道只放行 **1.0%**，
99% 的异体位落人审，等于废着。

R4 checkpoint（训练时每类加了康熙字头 + 字统网真刻本图）把同组差距拉开 12.6 倍到
**0.1734**。在 bench 组内定形样本（unseen 落在 ≥2 形异体组，n=691 / 520 字种）上扫阈值：

| 阈值 | 现役放行率/错 | R4+真刻本模板 放行率/错 |
|---|---|---|
| 0.03 | 1.0% / 0 | 91.2% / **2** |
| 0.08 | 0% | 83.8% / **2** |
| **0.12** | 0% | **69.9% / 0** |
...
取 0.12 不取 0.03：0.03 档有 2 条错，而这条通道错一次就把错标写进字形库（下游实例继承），
管线对自动放行的硬约束是 `match_precision ≥ 0.999`。0.12 是零错点，n=691 的 Wilson 下界 ≈99.5%。

**这个值与 checkpoint 绑定**：换回字体模板或旧 checkpoint 必须改回 0.03，否则这条通道全关。
重标脚本 `cache/exp_extglyph/calib_gap.py`。

## 2026-09-17 再重标：**r5 上线，回到 0.03**

r5（`cnn_candidates.DEFAULT_CKPT`，同配方只训 60 epoch）的同组 top1−top2 差是
**0.0482**，比 r4 的 0.1839 小 3.8 倍——**训得短，分数没被拉那么开**。
同一个 n=691 集上扫阈值（`calib_gap.py --model models/glyph_cnn_r5/best.pt`）：

| 阈值 | r4 放行率/错 | **r5 放行率/错** |
|---|---|---|
| 0.01 | 94.1% / 1 | 92.2% / 1 |
| 0.02 | 93.3% / 1 | 88.6% / 1 |
| **0.03** | 92.6% / 1 | **77.4% / 0** ← r5 零错点 |
| 0.05 | 91.3% / 1 | 43.8% / 0 |
| 0.08 | 87.7% / 1 | 9.8% / 0 |
| **0.12** | **76.1% / 0** ← r4 零错点 | **0.6% / 0**（通道等于关掉）|

所以 r5 取 **0.03**：零错、放行 77.4%，比 r4 的 76.1% 还略高。
零错点会随 checkpoint 移动，**换 checkpoint 必须重跑 `calib_gap.py` 重找**，
别沿用——沿用 0.12 到 r5 上就是把通道关掉且不报错。
"""
